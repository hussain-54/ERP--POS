...
describe("ModuleLauncherPage", () => {
  it("renders one card per registry module and opens POS inside routing", () => {
    ...
    expect(screen.getAllByText("Open Module →")).toHaveLength(39);
  ...
  it("searches module names, numbers, and child features", () => {
    ...
    expect(within(warehouseResults).getByText("06 WAREHOUSE / WMS")).toBeInTheDocument();
    expect(within(warehouseResults).queryByText(/^→ /)).not.toBeInTheDocument();
    ...
    expect(within(numberResults).getByText("02 POS / SALES")).toBeInTheDocument();
  });
});
