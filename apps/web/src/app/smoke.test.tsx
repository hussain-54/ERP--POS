...
describe("web foundation", () => {
  ...
  it("registers major module routes", () => {
    ...
    expect(sales?.status).toBe("implemented");
    expect(sales?.folder).toBe("pos");
    expect(sales?.children.some((c) => c.title === "Command Center")).toBe(true);
    expect(isComingSoonEngineSection(sales!)).toBe(false);
...
describe("nav structure", () => {
  ...
  it("reuses existing screens for shared children and keeps Coming Soon items as placeholders", () => {
    ...
    expect(child("02", "Command Center")?.path).toBe("/pos");
    expect(child("02", "Command Center")?.status).toBe("implemented");
    expect(child("02", "Returns & Exchange")?.status).toBe("implemented");
  ...
  it("opens POS with dedicated terminal chrome", () => {
    render(
      <MemoryRouter initialEntries={["/pos"]}>
        <AuthProvider>
          <ModuleWorkspace>
            <div data-testid="pos-terminal-content">Terminal</div>
          </ModuleWorkspace>
        </AuthProvider>
      </MemoryRouter>,
    );
    expect(screen.getByLabelText("POS navigation")).toBeInTheDocument();
    expect(screen.getByTestId("pos-terminal-content")).toBeInTheDocument();
  });
...
describe("responsive ERP shell", () => {
  ...
  it("opens POS with terminal navigation inside AppShell", () => {
